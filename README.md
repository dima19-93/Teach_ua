...
| **Автор** | Дмитро Сімко |
| **Проєкт** | Teach UA |
| **Ціль** | Організувати міграцію контейнеризованої інфраструктури вебзастосунку TeachUA з локальних віртуальних машин у хмарне середовище AWS (Elastic Container Service). Забезпечити високу безпеку, мережеву ізоляцію компонентів у приватних підмережах через NAT Gateway, відмовостійкість та керування артефактами деплою (Docker-образами в AWS ECR) і секретами за допомогою Terraform, AWS Parameter Store та декларативного пайплайну Jenkins. |
| **Завдання** | 1. Перенести запуск додатків фронтенду та бекенду в керований хмарний сервіс **AWS ECS (Fargate)** у приватних підмережах.<br>2. Забезпечити захищений вихід приватних ресурсів в інтернет за допомогою **AWS NAT Gateway**.<br>3. Забезпечити повну мережеву ізоляцію бази даних **MariaDB в AWS RDS** від зовнішнього світу.<br>4. Налаштувати балансування трафіку та безпечне з'єднання HTTPS через **AWS ALB (Application Load Balancer)**.<br>5. Організувати **Artifact Management**: налаштувати **Amazon ECR** як сховище артефактів, інтегрувати сканер **Trivy** для аудиту безпеки образів.<br>6. Налаштувати збереження конфіденційних даних та веб-хуків через **AWS Parameter Store**.<br>7. Впровадити канали моніторингу та сповіщень у **Discord** (окремо для результатів Trivy та алертингу CloudWatch при критичному навантаженні на CPU). |

### Цілі

**Основна мета:** Побудова захищеної та повністю автоматизованої хмарної інфраструктури розгортання та керування артефактами (Cloud Migration & Artifact Management) для проєкту TeachUA з нульовим ручним втручанням.

**Супутні цілі:**
- **Повна мережева ізоляція бази даних та контейнерів:** Розгортання обчислювальних ресурсів у приватних підмережах VPC. Використання **AWS NAT Gateway** для безпечного вихідного трафіку приватних компонентів без надання їм публічних IP-адрес. Реалізація файрволу на рівні AWS Security Groups для жорсткого обмеження доступу до СУБД (порт 3306 закритий від усього інтернету `0.0.0.0/0`, вхід дозволено строго для контейнерів ECS).
- **Перехід на хмарну концепцію артефактів:** Використання реєстру Amazon ECR як єдиного джерела для збереження та версіонування готових Docker-образів додатків замість важких локальних репозиторіїв.
- **Розподілений та безпечний моніторинг у Discord:** Налаштування каналів сповіщень, щоб результати збірок безпеки (Trivy) та інфраструктурні алерти (CloudWatch CPU Utilization) приходили в текстові чати.
- **Дотримання правил секретів та логів (Parameter Store):** Винесення всіх токенів (GitHub PAT, Discord Webhooks) з коду пайплайну та локальних налаштувань Jenkins безпосередньо в зашифровані параметри **AWS Parameter Store**.
...
| --- | --- | --- | --- | --- | --- |
| 1 | Ізоляція бази даних MariaDB в AWS RDS | Завершено | Дмитро Сімко | 13.09.2026 | Повністю видалено доступ `0.0.0.0/0` до порту 3306. |
| 2 | Керування артефактами в Amazon ECR | Завершено | Дмитро Сімко | 13.09.2026 | Конфігурація пушу Docker-образів фронтенду та бекенду в реєстр ECR. Вони виступають єдиними артефактами деплою. |
| 3 | Впровадження DevSecOps-сканера Trivy | Завершено | Дмитро Сімко | 13.09.2026 | Додано крок `Trivy Security Scanning` у `Jenkinsfile` для перевірки образів на вразливості. Налаштовано неблокуючий режим через флаг `--exit-code 0`. |
| 4 | Налаштування безпечного зв'язку та балансування | Завершено | Дмитро Сімко | 13.09.2026 | На балансувальнику ALB імпортовано та налаштовано роботу HTTPS-слухача (порт 443) для захищеного доступу користувачів. |
| 5 | Міграція секретів у AWS Parameter Store | Завершено | Дмитро Сімко | 13.09.2026 | Усі конфіденційні дані (DATASOURCE_PASSWORD, Discord Webhooks) перенесені в Parameter Store. Пайплайн зчитує їх через AWS CLI. |
| 6 | Інтеграція алертингу в Discord | Завершено | Дмитро Сімко | 13.09.2026 | Створено алерти CloudWatch для відстеження CPU контейнерів ECS (поріг > 80%). Налаштовано SNS та AWS Lambda функцію для відправки красивих карток у Discord. |

### Результати

**Виконані дії:**
- **Хмарна міграція на AWS ECS Fargate:** Повністю відмовилися від локальних віртуальних машин на користь контейнеризації AWS ECS Fargate. Компоненти додатків розгорнуті всередині приватної підмережі VPC, що виключає прямий доступ до них з інтернету.
- **Мережева ізоляція та AWS NAT Gateway:** Для безпечного виходу приватних контейнерів ECS в інтернет (наприклад, для зв'язку з AWS API) налаштовано **AWS NAT Gateway** у публічній підмережі. При цьому база даних MariaDB в RDS повністю закрита від зовнішнього світу на рівні Security Groups: порт 3306 приймає пакети строго від контейнерів додатків.
- **Керування артефактами деплою (Amazon ECR):** Кожна збірка Jenkins збирає Docker-образ та пушить в Amazon ECR. 
- **Впровадження DevSecOps автоматизації (Trivy):** У пайплайн Jenkins інтегровано крок автоматичного аудиту безпеки від Trivy. Для забезпечення безперервності деплою  використано флаг `--exit-code 0`, що дозволяє виводити знайдені CVE у консоль, але не зупиняти реліз сайту.
- **Маршрутизація трафіку та HTTPS:** Через Application Load Balancer (ALB) налаштовано безпечний вхід на сайт по HTTPS (порт 443). Існуючий робочий слухач балансувальника було успішно імпортовано в код інфраструктури.
- **Налаштування секретів у Parameter Store:** Повністю прибрано практику зберігання токенів у файлах репозиторію або налаштуваннях Jenkins. Пайплайн динамічно забирає розшифровані секрети (GitHub PAT, Discord URL) з AWS Parameter Store безпосередньо в пам'ять під час виконання команд.
- **Інфраструктурний алертинг через AWS Lambda:** Спроектовано систему безсерверного моніторингу заліза. Метрики CloudWatch при перевищенні порогу CPU > 80% відправляють сигнал в AWS SNS. Створена AWS Lambda функція перехоплює цей триггер, зчитує вебхук із Parameter Store та миттєво відправляє інформативні червоні (🚨 `ALARM`) або зелені (✅ `OK`) картки-повідомлення в окремий канал Discord.
...
| 1 | Помилка `failed to parse the image name` під час кроку сканування Trivy в Jenkins через неправильний URL | Виправлено синтаксичні опечатки в адресі ECR всередині `Jenkinsfile`: замінено некоректні символи `://{` на правильний доменний шлях `://{REPOSITORY}` | Завершено |
| 2 | Конфлікт `DuplicateListener` на порті 443 балансувальника ALB при запуску `terraform apply` | Наявний у хмарі робочий HTTPS-слухач було успішно імпортовано під контроль інфраструктурного коду за допомогою команди `terraform import module.app.aws_lb_listener.https <ARN_слухача>` | Завершено |


### Додатки та ресурси

- **Репозиторій проєкту:** [dima19-93/Teach_ua](https://github.com/dima19-93/Teach_ua/tree/devopscloud-infrastructure)

- Документація Terraform AWS Provider: https://registry.terraform.io/providers/hashicorp/aws/latest/docs

- Документація AWS Systems Manager Parameter Store: https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-parameter-store.html

- Документація Trivy в Jenkins (Aqua Security): https://trivy.dev/docs/latest/ecosystem/cicd/

- Створення тригерів AWS Lambda з SNS: https://docs.aws.amazon.com/lambda/latest/dg/with-sns.html
